 ...
 *
 *  Es wird in einem beliebigen Jahres-Interval ermittelt ob der 13. oefter auf einen
 *  Freitag als auf andere Tage faellt.
 */
 
public class DateProbabilityCalculator {

	/**
	 * Ermittelt, ob der 13. eines Monats oefter aus einen Freitag als auf andere
	 * Tage faellt.
	 *
	 *@param yearStart Jahr am Begin des Intervals (inklusive), in dem geprueft werden soll
	 *@param yearEnd Jahr am Ende des Intervals (inklusive), in dem geprueft werden soll
	 *@param dayToBegin Wochentag des 01.01. von {@link yearStart} von 0=Montag bis 6=Sonntag
	 *
	 *@return true, wenn dem so ist
	 */	
	public boolean isMoreOftenFriday(int yearStart, int yearEnd, int dayToBegin) {
		
		boolean itIs = false;
	
		int dayOfTheFirst = dayToBegin;
		...
		if (daysIn400y%7 == 0) {
			...
			//Ueber die Jahre iterieren
			for (int year=yearStart; year<=yearEnd; year+=1) {
	...
	public void run() {
		...
		System.out.println();
		System.out.println("Der 13. fällt zwischen 2000 und 2012 öfter auf "
				+"einen Freitag als auf andere Tage: das ist "+isMoreOftenFriday(2000, 2012, 5));
		System.out.println();
	...
	public static void main(String[] args) {
		...
		DateProbabilityCalculator calculator = new DateProbabilityCalculator();
		calculator.run();
